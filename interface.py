...
def ctr_city_check():
    global ctr_name, city_name, city_checked, city_checked

    if not city_name or not ctr_name or city_name == '' or ctr_name == '' :
        messagebox.showerror('Error', 'Veuillez sélectionner une ville / un pays dans les listes avant de lancer la recherche')
        return(False)
...
def city_listbox_select():
    global city_name, city_checked
    city_name = city_list.get()
    city_checked = True

def ctr_search_get():
    ctr_list.delete(0, END) # Clear previous results
    search_term = ctr_entry.get().lower()
    if search_term:
        country_names = [name.lower() for name in ctr_to_iso().keys()]
        matches = get_close_matches(search_term, country_names, n=5, cutoff=0.5)
...
def ctr_listbox_select():
    global ctr_name, ctr_checked

    ctr_name = ctr_list.get()
    ctr_checked = True
...
space_rain_snow = CTkLabel(right_side, fg_color=color_frame, corner_radius=10, text=f'Précipitations (mm) : {rain}   /   Neige (mm) : {snow}', font=('Monserrat', 15, 'bold'), anchor=W)
...
ctr_button = CTkButton(ctr_search_frame, text='VALIDER', corner_radius=10, font=('Monserrat', 15, 'bold'), hover_color='#B6B6B6', text_color='#59608C', fg_color='white', width=80, command=city_search_get) # BOUTON VALIDER PAYS
...
city_button = CTkButton(city_search_frame, text='VALIDER', corner_radius=10, font=('Monserrat', 15, 'bold'), hover_color='#B6B6B6', text_color='#59608C', fg_color='white', width=80) # BOUTON VALIDER CITY
city_button.pack(fill=BOTH, pady=0, padx=0, side=RIGHT)

city_list = CTkListbox(menu, width=2, command=city_listbox_select)
...
date = CTkLabel(right_side, text=f'{datetime.datetime.now().strftime("%d/%m/%Y %H:%M")}     Sunrise / Sunset : {sunrise} / {sunset}', font=('Monserrat', 12), text_color='white')
date.pack(side=LEFT)
