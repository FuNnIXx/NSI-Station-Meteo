...
def ISO_CTR(Pays):
    ...
    recherche_actuelle = Pays

    while True:
        correspondances = difflib.get_close_matches(recherche_actuelle, ISO.keys(), n=5, cutoff=0.5)
        ...
        for index, nom_pays in enumerate(correspondances):
            print(f" {index + 1} -> {nom_pays}")
        ...
        if choix.lower() == 'x':
            recherche_actuelle = input("Entrez le nom du pays : ")
            continue 
        ...
        try:
            ...
            if 0 <= index_choisi < len(correspondances):
                pays_selectionne = correspondances[index_choisi]
                print(f"Sélectionné : {pays_selectionne}")
                return ISO[pays_selectionne] 
            else:
