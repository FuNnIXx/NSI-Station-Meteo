...
# Le dictionnaire est maintenant une constante globale
ISO_DATA = {
    ...
    "Comoros": "KM", "Congo": "CG", "Congo, Democratic Republic": "CD",
    ...
    "Haiti": "HT", "Heard Island & Mcdonald Islands": "HM", "Holy See (Vatican City State)": "VA",
    ...
    "Korea": "KR", "North Korea": "KP", "Kuwait": "KW", "Kyrgyzstan": "KG",
    ...
    "Micronesia, Federated States Of": "FM", "Moldova": "MD", "Monaco": "MC",
    ...
    "South Africa": "ZA", "South Georgia And Sandwich Isl.": "GS", "Spain": "ES",
    ...
    "United Kingdom": "GB", "United States": "US", "United States Outlying Islands": "UM",
...
def get_iso_code(country_input):
    """
    Renvoie le code ISO (ex: 'FR') ou None si le pays n'est pas trouvé.
    """
    # 1. Recherche exacte
    for name, code in ISO_DATA.items():
        if name.lower() == country_input.lower():
            return code

    # 2. Recherche approximative (correction d'orthographe)
    matches = difflib.get_close_matches(country_input, ISO_DATA.keys(), n=1, cutoff=0.5)
    if matches:
        return ISO_DATA[matches[0]]
